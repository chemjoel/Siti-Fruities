...
export default function GreekYogurtParfaitsPage() {
  ...
  const [vvipWithBanana, setVvipWithBanana] = useState<boolean>(true);
  ...
  const vvipBananaSurcharges: Record<string, number> = {
    '330ml': 200,
    '500ml': 300,
    '550ml': 500,
    '1L': 1000,
    '2L': 1500,
    '5L': 0 // Not specified, disabled in UI
  };

  const vvipUnitPrice = vvipBasePrices[vvipSize] + (!vvipWithBanana && vvipSize !== '5L' ? vvipBananaSurcharges[vvipSize] : 0);
  ...
  const handleAddVvip = () => {
    addItem({
      ...
      options: [
        ...
        { name: 'Banana Option', value: vvipWithBanana ? 'With Banana' : 'Without Banana' }
      ]
    });
    setVvipQty(1);
  };
  ...
  const [vipWithBanana, setVipWithBanana] = useState<boolean>(true);
  ...
  const vipBananaSurcharges: Record<string, number> = {
    '330ml': 200,
    '500ml': 300,
    '550ml': 500,
    '1L': 1000
  ...
  const vipUnitPrice = vipBasePrices[vipSize] + (!vipWithBanana ? vipBananaSurcharges[vipSize] : 0);
  ...
  const handleAddVip = () => {
    addItem({
      ...
      options: [
        ...
        { name: 'Banana Option', value: vipWithBanana ? 'With Banana' : 'Without Banana' }
      ]
    });
    setVipQty(1);
  };
  ...
  const fruitOptions = ['Apple', 'Banana', 'Coconut', 'Grapes', 'Strawberries', 'Kiwi'];
  ...
  // Base pricing uses standard size rate + banana removal check
  ...
  const customBananaSurcharges: Record<string, number> = {
    '330ml': 200,
    '500ml': 300,
    '550ml': 500,
    '1L': 1000,
  ...
  const hasBanana = selectedFruits.includes('Banana');
  const customUnitPrice = customBasePrices[customSize] + (!hasBanana && customSize !== '5L' ? customBananaSurcharges[customSize] : 0);
              ...
              <div className="w-full md:w-1/2 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {['Greek Yogurt', 'Apple', 'Banana', 'Coconut', 'Grapes', 'Strawberries', 'Kiwi', 'Granola with rolled oats', 'Raisins', 'Cashew nuts'].map((ing) => (
                    ...
                    {Object.entries({
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          // Force banana option back to true if selecting 5L
                          if (size === '5L') setVvipWithBanana(true);
                        }}
                ...
                {/* Banana Toggle Choice */}
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Banana Customization</label>
                    {vvipSize !== '5L' && !vvipWithBanana && (
                      <span className="text-[10px] bg-amber-500/10 text-amber-600 font-bold px-2 py-0.5 rounded border border-amber-500/20">
                        Surcharge: +{formatPrice(vvipBananaSurcharges[vvipSize])}
                      </span>
                    )}
                  </div>
                  {vvipSize === '5L' ? (
                    <div className="bg-muted border border-border p-3 rounded-xl flex items-start gap-2.5">
                      <Info className="w-4 h-4 text-muted-foreground shrink-0 mt-0.5" />
                      <p className="text-xs text-muted-foreground font-medium leading-normal">
                        Banana-removal customization and surcharge is not available for the 5-litre Twa Bowl size.
                      </p>
                    </div>
                  ) : (
                    <div className="flex gap-2">
                      <button
                        onClick={() => setVvipWithBanana(true)}
                        className={`px-4 py-2 rounded-full text-sm font-semibold border transition-all ${
                          vvipWithBanana
                            ? 'bg-primary border-primary text-white shadow-md'
                            : 'bg-white border-border text-foreground hover:border-primary/50 hover:bg-primary/5'
                      ...
                      <button
                        onClick={() => setVvipWithBanana(false)}
                        className={`px-4 py-2 rounded-full text-sm font-semibold border transition-all ${
                          !vvipWithBanana
                            ? 'bg-primary border-primary text-white shadow-md'
                            : 'bg-white border-border text-foreground hover:border-primary/50 hover:bg-primary/5'
            ...
            <div className="flex flex-col md:flex-row gap-8 lg:gap-12 items-center">
              ...
              <div className="w-full md:w-1/2 space-y-6">
                <div>
                  ...
                  <p className="text-muted-foreground mt-2">
                    Our classic parfait layered with fresh apples, banana, grapes, crunchy granola, raisins, and premium cashew nuts.
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-muted-foreground uppercase tracking-wider block">Ingredients Included</label>
                  <div className="flex flex-wrap gap-1.5">
                    {['Greek Yogurt', 'Apple', 'Banana', 'Coconut', 'Grapes', 'Granola with rolled oats', 'Raisins', 'Cashew nuts'].map((ing) => (
                      <span key={ing} className="px-2.5 py-1 bg-white border border-border/80 text-[11px] font-semibold rounded-lg text-foreground/80 flex items-center gap-1">
                        <Check className="w-3 h-3 text-secondary" /> {ing}
                      </span>
                    ))}
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    {!vipWithBanana && (
                      <span className="text-[10px] bg-amber-500/10 text-amber-600 font-bold px-2 py-0.5 rounded border border-amber-500/20">
                        Surcharge: +{formatPrice(vipBananaSurcharges[vipSize])}
                      </span>
                    )}
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setVipWithBanana(true)}
                      className={`px-4 py-2 rounded-full text-sm font-semibold border transition-all ${
                        vipWithBanana
                          ? 'bg-primary border-primary text-white shadow-md'
                          : 'bg-white border-border text-foreground hover:border-primary/50 hover:bg-primary/5'
                    ...
                    <button
                      onClick={() => setVipWithBanana(false)}
                      className={`px-4 py-2 rounded-full text-sm font-semibold border transition-all ${
                        !vipWithBanana
                          ? 'bg-primary border-primary text-white shadow-md'
                          : 'bg-white border-border text-foreground hover:border-primary/50 hover:bg-primary/5'
                ...
                <div className="bg-muted/30 border border-border p-6 rounded-2xl flex flex-col justify-between space-y-6">
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2 text-sm text-foreground/80 font-medium">
                      ...
                      <div className="flex justify-between">
                        <span>Banana option:</span>
                        <span className="font-bold">{hasBanana ? 'Included' : 'Removed'}</span>
                      </div>

                      {!hasBanana && customSize !== '5L' && (
                        <div className="flex justify-between text-amber-600 font-bold">
                          <span>Banana Removal Surcharge:</span>
                          <span>+{formatPrice(customBananaSurcharges[customSize])}</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="bg-secondary/5 border border-secondary/10 p-3 rounded-xl flex items-start gap-2">
                      ...
                      <p className="text-[11px] text-muted-foreground font-medium leading-relaxed">
                        <strong>Custom Pricing Notice:</strong> Parfait is priced at standard size base rates. Removing banana applies the standard surcharge. Final customization details are verified at order confirmation.
                      </p>
                    </div>
                  </div>
